...
def angular_isf_kernel(indices, get_frame, system_offset, system_size, angle_norm):
    t0, t1, = indices
    theta0 = get_frame(t0)['angle']
    theta1 = get_frame(t1)['angle']
    exp_theta = np.exp(1j * 2 * np.pi * (theta1 - theta0) / angle_norm)
    return np.real(np.add.reduceat(exp_theta, system_offset[:-1]) / system_size)
...
def fused_msd_kernel(indices, get_frame, system_id, system_size, system_offset):
    ...
    dr = r1 - r0
    dr -= (np.add.reduceat(dr, system_offset[:-1], axis=0) / system_size[:, None])[system_id]  # subtract off the mean drift of each system
    msd = np.add.reduceat(np.sum(dr ** 2, axis=-1), system_offset[:-1]) / system_size
    # msd = np.bincount(system_id, weights=np.sum(dr ** 2, axis=-1)) / system_size
    dtheta = (theta1 - theta0)
    angular_msd = np.add.reduceat(dtheta ** 2, system_offset[:-1]) / system_size
...
def compute_rotational_isf(data, save_path=None, overwrite=False, n_workers=20):
    if save_path is not None and os.path.exists(save_path) and not overwrite:
        return np.load(save_path)['rot_isf'], np.load(save_path)['t']
    bins = LagBinsPseudoLog.from_source(data.trajectory)
    res = run_binned(
        angular_isf_kernel,
        data.trajectory,
        bins,
        kernel_kwargs={'system_offset': data.system_offset, 'system_size': data.system_size, 'angle_norm': 2 * np.pi / data.n_vertices_per_particle},
        show_progress=True,
        n_workers=n_workers
    )
    if save_path is not None:
        np.savez(save_path, rot_isf=res.mean, t=bins.values())
    return res.mean, bins.values()
