...
pos = jnp.asarray(pos)[0]
vel = jnp.asarray(vel)[0]
mass = jnp.asarray(mass)[0]
rad = jnp.asarray(rad)[0]
box_size = jnp.asarray(box_size)[0]
e_int = jnp.asarray(e_int)[0]
ids = jnp.asarray(ids)[0]

mats = [jd.Material.create("elastic", young=e_int, poisson=0.2)]
