...
def stress_acf_kernel(indices, get_frame, stress_mean):
    t0, t1 = indices
    s_x_0 = get_frame(t0)['stress_tensor_total_x'] - stress_mean[:, 0]
    s_y_0 = get_frame(t0)['stress_tensor_total_y'] - stress_mean[:, 1]
    s_x_1 = get_frame(t1)['stress_tensor_total_x'] - stress_mean[:, 2]
    s_y_1 = get_frame(t1)['stress_tensor_total_y'] - stress_mean[:, 3]
...
def compute_stress_acf(data, save_path=None, subtract_mean_stress=True, overwrite=False, n_workers=20):
    ...
    # if save_path is not None:
        # np.savez(save_path, shear_modulus=res.mean * area / np.mean(temp, axis=0), t=bins.values())
    # return res.mean * area / np.mean(temp, axis=0), bins.values()
    return res.mean, temp, area, stress, bins.values()
