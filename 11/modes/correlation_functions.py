...
@requires_fields("stress_tensor_total_x", "stress_tensor_total_y")
def stress_acf_kernel(indices, get_frame, stress_mean):
    t0, t1 = indices
    s_x_0 = get_frame(t0)['stress_tensor_total_x'] - stress_mean[:, [0, 1]]
    s_y_0 = get_frame(t0)['stress_tensor_total_y'] - stress_mean[:, [2, 3]]
    s_x_1 = get_frame(t1)['stress_tensor_total_x'] - stress_mean[:, [0, 1]]
    s_y_1 = get_frame(t1)['stress_tensor_total_y'] - stress_mean[:, [2, 3]]
    return [s_x_1[:, 0] * s_x_0[:, 0], s_x_1[:, 1] * s_x_0[:, 1], s_y_1[:, 0] * s_y_0[:, 0], s_y_1[:, 1] * s_y_0[:, 1]]

@requires_fields("pos", "vel")
def einstein_helfand_kernel(indices, get_frame, system_offset, mass):
    t0, t1 = indices
    pos_0 = get_frame(t0)['pos']
    pos_1 = get_frame(t1)['pos']
    vel_0 = get_frame(t0)['vel']
    vel_1 = get_frame(t1)['vel']
    return 0.5 * np.add.reduceat(
        np.array([
            vel_1[:, 0] * pos_1[:, 0] - vel_0[:, 0] * pos_0[:, 0],
            vel_1[:, 0] * pos_1[:, 1] - vel_0[:, 0] * pos_0[:, 1],
            vel_1[:, 1] * pos_1[:, 0] - vel_0[:, 1] * pos_0[:, 0],
            vel_1[:, 1] * pos_1[:, 1] - vel_0[:, 1] * pos_0[:, 1]
        ]) * mass,
        system_offset[:-1],
        axis=1
    ) ** 2
...
def compute_stress_acf(data, save_path=None, subtract_mean_stress=True, overwrite=False, n_workers=20):
    if save_path is not None and os.path.exists(save_path) and not overwrite:
        return np.load(save_path)['shear_modulus'], np.load(save_path)['t']
    temp = np.array([data.trajectory[i].temperature for i in range(data.trajectory.num_frames())])
    area = np.prod(data.box_size, axis=-1)
    stress = np.array(
        [
            np.column_stack(
                [data.trajectory[i].stress_tensor_total_x, data.trajectory[i].stress_tensor_total_y]
                ) for i in range(data.trajectory.num_frames())
        ]
    )
    if subtract_mean_stress:
        ...
        mean_stress = np.zeros(shape=(data.n_systems(), 4))
    bins = LagBinsPseudoLog.from_source(data.trajectory)
    res = run_binned(
        stress_acf_kernel,
        data.trajectory,
        bins,
        kernel_kwargs={'stress_mean': mean_stress},
        show_progress=True,
        n_workers=n_workers
    )
    # if save_path is not None:
...
def compute_einstein_helfand_stress_acf(data, n_workers=20):
    temp = np.array([data.trajectory[i].temperature for i in range(data.trajectory.num_frames())])
    area = np.prod(data.box_size, axis=-1)
    bins = LagBinsPseudoLog.from_source(data.trajectory)
    res = run_binned(
        einstein_helfand_kernel,
        data.trajectory,
        bins,
        kernel_kwargs={'system_offset': data.system_offset.copy(), 'mass': data.mass.copy()},
        show_progress=True,
        n_workers=n_workers
    )
    return res.mean / (area * np.mean(temp, axis=0)), bins.values()