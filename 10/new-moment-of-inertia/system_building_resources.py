...
def build_rigid_bumpy_system_from_radii(radii: np.ndarray, which: str, mu_eff: float | list[float], nv: int | list[int], packing_fraction: float, add_core: bool | str = False, cap_nv: float = np.inf, inertia_assignment: str = 'points', n_duplicates: int = 1, draw_figures: bool = False):
    """
    ...
    inertia_assignment: str defining how to calculate the inertia of the particles: "points" uses the basic point-mass formula, "uniform" uses a uniform-area density
