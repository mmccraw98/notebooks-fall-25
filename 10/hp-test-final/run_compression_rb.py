...
import time
import shutil
from correlation_functions import compute_msd, compute_pair_correlation_function, compute_rotational_msd
...
if __name__ == "__main__":
    ...
    which = 'small'
    packing_fraction = 0.70
    phi_increment = 1e-2
    packing_fraction_target = 0.84
    dt = 5e-2

    for temp in [1e-4, 1e-5, 1e-6]:
        root = f"/home/mmccraw/dev/data/10-03-25/rb_compression_T_{temp:.3e}/"
        ...
        for i, _rb in enumerate(split_systems(rb)):
            if _rb.packing_fraction[0] < packing_fraction - phi_increment:
                ...
                while _rb.packing_fraction[0] < packing_fraction - phi_increment:
                    _rb.set_velocities(temp, np.random.randint(0, 1e9))
        ...
        while True:
            ...
            rb.set_velocities(temp, np.random.randint(0, 1e9))
            rb.set_neighbor_method(NeighborMethod.Cell)
            set_standard_cell_list_parameters(rb, 0.3)
                ...
                np.array(
                    [
                        np.add.reduceat(  # sum over all vertices within each system
                            rb.trajectory[i].overlaps, rb.vertex_system_offset[:-1]
                        , axis=0)
            ...
            # scale by the vertex diameter
            dimless_overlaps = mean_overlaps / (2.0 * rb.vertex_rad[rb.vertex_system_offset[:-1]])

            # calculate the average temperature in each system
            mean_temp = np.mean([rb.trajectory[i].temperature for i in range(rb.trajectory.num_frames())], axis=0)

            # calculate the average pressure in each system
            mean_p = np.mean([rb.trajectory[i].pressure for i in range(rb.trajectory.num_frames())], axis=0)

            # calculate the msd
            msd, t = compute_rotational_msd(rb)

            # calculate the pair correlation function
            g, r = compute_pair_correlation_function(rb)
