...
if __name__ == "__main__":
    ...
    dynamics_root = os.path.join(data_root, "dynamics-2")
    if not os.path.exists(dynamics_root):
        os.makedirs(dynamics_root)
    for particle_type in os.listdir(jamming_root):
        for delta_phi in -np.logspace(-5, -1, 20):
            ...
            phi = np.sum(state.volume) / np.prod(old_system.domain.box_size)
            scale = ((phi + delta_phi) / phi) ** (1 / state.dim)
            ...
            n_duplicates = 10
            temperatures = np.linspace(1e-5, 2e-5, n_duplicates)
            duplicated_state = jd.State.stack([state for _ in range(n_duplicates)])
            duplicated_system = jd.System.stack([system for _ in range(n_duplicates)])
            key = jax.random.PRNGKey(np.random.randint(0, 1e9))
            duplicated_state = assign_random_velocities_temperature(duplicated_state, key, temperatures)

            _, ids = jnp.unique(duplicated_state.ID, return_index=True, axis=1)
            ...
            pe = np.zeros((n_snapshots * n_runs, n_duplicates))
            ke_r = np.zeros((n_snapshots * n_runs, n_duplicates))
            ke_t = np.zeros((n_snapshots * n_runs, n_duplicates))
            T = np.zeros((n_snapshots * n_runs, n_duplicates))
            for i in range(n_runs):
                state, system, (state_traj, system_traj) = system.trajectory_rollout(
                    duplicated_state, duplicated_system, n=n_snapshots, stride=save_stride
                )

                energies_TB = jax.vmap(  # time
                    jax.vmap(energies_one, in_axes=(0, 0)),  # batch
                    in_axes=(0, 0),
                )(state_traj, system_traj)

                _pe, _ke_t, _ke_r, _T = energies_TB
                pe[i * n_snapshots: (i + 1) * n_snapshots] = _pe
                ke_r[i * n_snapshots: (i + 1) * n_snapshots] = _ke_r
                ke_t[i * n_snapshots: (i + 1) * n_snapshots] = _ke_t
                T[i * n_snapshots: (i + 1) * n_snapshots] = _T

            dynamics_path = os.path.join(dynamics_root, particle_type, f'dphi-{abs(delta_phi)}')
            if not os.path.exists(dynamics_path):
                os.makedirs(dynamics_path)
            with h5py.File(os.path.join(dynamics_path, 'data.h5'), 'w') as f:
                names = ['pe', 'ke_t', 'ke_r', 'T']
                arrs = [pe, ke_t, ke_r, T]
