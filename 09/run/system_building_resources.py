...
def build_rigid_bumpy_system_from_radii(radii: np.ndarray, which: str, mu_eff: float | list[float], nv: int | list[int], packing_fraction: float, add_core: bool = False, cap_nv: float = np.inf, n_duplicates: int = 1):
    """
    ...
    for i, disk in enumerate(split_systems(disk)):
        ...
        rb = RigidBumpy()
        rb.using_core = add_core and mu_eff[i] > 0
        if rb.using_core:
